...
addDam = function() {
  tab = yasgui.addTab(
    true, // set as active tab
    { requestConfig: {"endpoint": 'https:\/\/'+app_config.data_host+'/sparql'}, name: "Dams" }
  );
  tab.setQuery(`PREFIX usgs: <http:\/\/`+app_config.data_host+`\/lod\/usgs\/ontology\/>
PREFIX gnis: <http:\/\/`+app_config.data_host+`\/lod\/gnis\/ontology\/>
PREFIX rdfs: <http:\/\/www.w3.org\/2000\/01\/rdf-schema#>
PREFIX rdf: <http:\/\/www.w3.org\/1999\/02\/22-rdf-syntax-ns#>
PREFIX qudt: <http:\/\/qudt.org\/schema\/qudt\/>
PREFIX onto:<http://www.ontotext.com/> 
SELECT ?dam ?name ?citation {
  ?dam rdf:type usgs:Dam .
  ?dam gnis:officialName ?name .
  ?dam gnis:citation ?citation .
}
`);
};