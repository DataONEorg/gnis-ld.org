...
const PD_LIB_SERVER = path.join(PD_LIB, 'server');
...
k_app.use('/sparql', createProxyMiddleware({
...
k_app.use('/data', createProxyMiddleware({
  ...
  onProxyReq: (proxyReq, req, res) => {
    // Add the query parameters to the GraphDB request since it can't be done in the
    // pathRewrite expression
...
// static routing
console.log(path.join(PD_LIB_SERVER, '_scripts'));
...
const negotiate_feature = (d_req, d_res, f_next) => {
  ...
  let {
	...
	// redirection
	let p_redirect = `//stko-kwg.geog.ucsb.edu/browse/#http://${S_DATA_HOST}${S_DATA_PATH}/${s_dataset}/${s_group}/${s_thing}`;
