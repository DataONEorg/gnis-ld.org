...
k_app.get('/plain/:resource', (d_req, d_res) => {
	d_res.sendFile(path.join(PD_RESOURCES, d_req.params.resource));
});
...
const negotiate_feature = (d_req, d_res, f_next) => {
	...
	// redirection
	let p_redirect = `http://phuzzy.link/browse/${D_URL_ENDPOINT.host}${D_URL_ENDPOINT.pathname}#http://${S_DATA_HOST}${S_DATA_PATH}/${s_dataset}/${s_group}/${s_thing}`;
